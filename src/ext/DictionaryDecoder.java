...
public class DictionaryDecoder {
	...
	public DictionaryDecoder(String ontoFile) throws Exception {
		...
		for (Map.Entry<String, Long> e: d.getMappings().entrySet()) {
			...
			if (previous!=null) {
				System.err.println("Found duplicate mapping: " + e.getKey() + "->" + e.getValue());
			}
		}
	...
	public void decode(Collection<Triple> c, OutputStream out) throws IOException {
	 ...
	 * @param dept 
	   ...
	   */
	  private String _getId(int classType, int index, int university, int dept, int publication) {
	    String id;
	   

	    switch (classType) {
	      case Generator.CS_C_UNIV:
	        id = "http://www." + _getRelativeName(classType, university) + ".edu";
	        break;
	      case Generator.CS_C_DEPT:
	        ...
	        break;
	      default:
	        id = _getId(Generator.CS_C_DEPT, dept, university, dept, 0) + Generator.ID_DELIMITER +
	    ...
	    if (publication >0) {
	    	id+= Generator.ID_DELIMITER + Generator.CLASS_TOKEN[Generator.CS_C_PUBLICATION] + (publication-1);
	    }
	  ...
	  private String _getName(int classType, int index, int university, int dept) {
	    ...
	    switch (classType) {
	      case Generator.CS_C_UNIV:
	        ...
	        break;
	      case Generator.CS_C_DEPT:
	        name = _getRelativeName(classType, index) + Generator.INDEX_DELIMITER + "" + university;
	        break;
	      ...
	      case Generator.CS_C_RESEARCH:
	        name = _getRelativeName(classType, index) + Generator.INDEX_DELIMITER + ""+ dept;
	        break;
	      default:
	        name = _getRelativeName(classType, index) + Generator.INDEX_DELIMITER +
	              "" + dept+ Generator.INDEX_DELIMITER + "" + university;
	        break;
	    }
	  ...
	  private String _getEmail(int classType, int index, int university, int dept) {
	    ...
	    switch (classType) {
	      ...
	      case Generator.CS_C_DEPT:
	        email += _getRelativeName(classType, index) + "@" +
	            _getRelativeName(classType, index) + "." +
	            _getRelativeName(Generator.CS_C_UNIV,university) + ".edu";
	        break;
	      default:
	        email += _getRelativeName(classType, index) + "@" +
