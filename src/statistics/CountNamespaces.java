...
public class CountNamespaces {
			...
			try {
				...
				for (String term : terms) {
					...
					if (term.startsWith("<")) { // URI
						/*
						 * if (term.indexOf('#') > 0) { String namespace =
						 * term.substring(1, term.indexOf('#'));
						 * oKey.set(namespace);
						 *
						 * } else if (term.lastIndexOf('/') > 0) { String
						 * namespace = term.substring(1, term.lastIndexOf('/'));
						 * oKey.set(namespace); } else { oKey.set(term); }
						 */
				...
				/*
				 * if (set.size() > 1) { String term1 = set.pollFirst(); String
				 * term2 = set.pollFirst(); oKey.set(term1 + " - " + term2);
				 * context.write(oKey, output);
				 *
				 * if (set.size() == 1) { String term3 = set.pollFirst();
				 * oKey.set(term1 + " - " + term3); context.write(oKey, output);
				 * oKey.set(term2 + " - " + term3); context.write(oKey, output);
				 * } }
				 */
	...
	private void run(String[] args) throws IOException, InterruptedException,
		...
		Job job = new Job();
		job.setJobName("Counting namespaces");
		job.setJarByClass(CountNamespaces.class);
