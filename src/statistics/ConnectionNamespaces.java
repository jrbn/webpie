...
public class ConnectionNamespaces {
	...
	public static void main(String[] args) throws IOException,
		...
		new ConnectionNamespaces().run(args);
	}
		...
		protected void map(Text key, Text value, Context context)
			...
			try {
				...
				if (set.size() > 1) {
					...
					String term1 = set.pollFirst();
					String term2 = set.pollFirst();
					oKey.set(term1 + " - " + term2);
					context.write(oKey, output);

					if (set.size() == 1) {
						context.getCounter("Triples", "3 sets").increment(1);
						String term3 = set.pollFirst();
						oKey.set(term1 + " - " + term3);
						context.write(oKey, output);
						oKey.set(term2 + " - " + term3);
						context.write(oKey, output);
					}
				}
	...
	private void run(String[] args) throws IOException, InterruptedException,
		...
		job.setJarByClass(ConnectionNamespaces.class);
