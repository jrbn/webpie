...
public class CreateLUBMDatasetMapper extends
	...
	protected static Logger log = LoggerFactory
			.getLogger(CreateLUBMDatasetMapper.class);

	protected void map(LongWritable key, Text value, Context context)
			throws IOException, InterruptedException {
		// log.debug("map code: startingUniversity = " + startingUniversity +
		...
		// log.debug("currentUniversity = " + currentUniversity + " limit = " +
		// (startingUniversity + numberUniversity));
		while (currentUniversity < (startingUniversity + numberUniversity)) {
			Generator g = new Generator(1, currentUniversity, 0); // 1
																	// university,
																	// start at
																	// index 0,
																	// use 0 for
