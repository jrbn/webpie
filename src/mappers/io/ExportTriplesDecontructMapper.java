...
public class ExportTriplesDecontructMapper extends Mapper<BytesWritable, BytesWritable, LongWritable, BytesWritable> {

	private static Logger log = LoggerFactory.getLogger(ExportTriplesDecontructMapper.class);
	...
	protected void map(BytesWritable key, BytesWritable value, Context context)
		...
		if (key.getBytes()[0] == 1) { //Dictionary entry
			...
			if (!commonResources.containsKey(bKey)) {
				...
				System.arraycopy(bValue, 0, textValue.getBytes(), 1, value.getLength());
	...
	private void prepareCommonOutputValues(long tripleId, byte position, long resource) {
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		if (commonResources == null) {
			String joinCommonResources = context.getConfiguration().get("joinCommonResources");
			commonResources = FilesDictReader.readInvertedCommonResources(context.getConfiguration(), new Path(joinCommonResources));
			
			//Load also the already known URIs
			Map<String,Long> preloadedURIs = TriplesUtils.getInstance().getPreloadedURIs();
			if (preloadedURIs != null) {
				Iterator<Entry<String, Long>> itr = preloadedURIs.entrySet().iterator();
				while (itr.hasNext()) {
					Entry<String,Long> entry = itr.next();
					commonResources.put(entry.getValue().longValue(), entry.getKey());
				}
			}
		}
