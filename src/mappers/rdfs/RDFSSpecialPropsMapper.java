...
public class RDFSSpecialPropsMapper extends
		Mapper<TripleSource, Triple, BytesWritable, LongWritable> {

	protected static Logger log = LoggerFactory
			.getLogger(RDFSSpecialPropsMapper.class);
	...
	public void map(TripleSource key, Triple value, Context context)
		...
		if (value.getPredicate() == TriplesUtils.RDF_TYPE) {
			if ((value.getObject() == TriplesUtils.RDFS_LITERAL || literalSubclasses
					.contains(value.getObject()))) {
				bKey[0] = 0;
			...
			if ((value.getObject() == TriplesUtils.RDFS_MEMBER || memberProperties
					.contains(value.getObject()))) {
				bKey[0] = 1;
				NumberUtils.encodeLong(bKey, 1, value.getSubject());
				oKey.set(bKey, 0, 9);
	...
	public void setup(Context context) throws IOException {
		...
		if (resourceSubclasses == null) {
			...
			FilesTriplesReader.loadSetIntoMemory(resourceSubclasses, context,
					"FILTER_ONLY_RESOURCE_SUBCLAS_SCHEMA", -1);
		}

		if (literalSubclasses == null) {
			literalSubclasses = new HashSet<Long>();
			FilesTriplesReader.loadSetIntoMemory(literalSubclasses, context,
					"FILTER_ONLY_LITERAL_SUBCLAS_SCHEMA", -1);
		}
	}
}
