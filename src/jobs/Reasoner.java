...
import jobs.blocks.OWL2BuildLists;
import jobs.blocks.OWL2HasKey;
import jobs.blocks.OWL2IntersectionOf;
import jobs.blocks.OWL2PropertyChainAxiom;
...
public class Reasoner extends Configured implements Tool {
	...
	if (fragment == ExecutionBlock.FRAGMENT_OWL
	    ...
	    block = new OWLNotRecursiveBlock();
	    blocksToExecute.add(block);
	...
	if (fragment == ExecutionBlock.FRAGMENT_OWL2) {
	    blocksToExecute.add(new OWL2BuildLists());
	    blocksToExecute.add(new OWL2PropertyChainAxiom());
	    blocksToExecute.add(new OWL2HasKey());
