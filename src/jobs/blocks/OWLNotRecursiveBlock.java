...
public class OWLNotRecursiveBlock extends ExecutionBlock {
	...
	public void performJobs(int executionStep) throws IOException,
		...
		if (getNotFilteredDerivation() > 0) setHasDerived(true);
		...
		if (outputSize > 0
			...
			setFilteredDerivation(deleteDuplicatedTriples(pool.toString(),
					tmpDir, "FILTER_ONLY_HIDDEN", outputDir, getFilterFromStep(),
					true, false, true));
		...
		Path synTableDir = new Path(tmpDir, "dir-synonymstable");
