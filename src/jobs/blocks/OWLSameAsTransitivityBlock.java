...
public class OWLSameAsTransitivityBlock extends ExecutionBlock {
	...
	while (derivedSynonyms) {
	    ...
	    job.setJarByClass(OWLSameAsTransitivityBlock.class);

	    job.getConfiguration().setInt("maptasks", numMapTasks);
	    job.setNumReduceTasks(numReduceTasks);
	    job.setJobName("OWL reasoner: build the synonyms table from same as triples - step "
		    + derivationStep++);
	    job.setInputFormatClass(FilesTriplesReader.class);
	    FilesTriplesReader.addInputPath(job, new Path(inputDir));
