...
public class OWL2MergeListsReducer extends
	...
	public void reduce(LongWritable key, Iterable<BytesWritable> values,
		...
		while (itr.hasNext()) {
			...
			System.arraycopy(value.getBytes(), 1, bValue, 0, value.getLength() - 1);
		...
		while (itrHeads.hasNext()) {
			...
			if (tails.size() > 0) {
				...
				while (itrTails.hasNext()) {
					...
					NumberUtils.encodeLong(oKey.getBytes(), 0, NumberUtils.decodeLong(bValueTail, 0));
					NumberUtils.encodeLong(oKey.getBytes(), 8, NumberUtils.decodeLong(bValue, 0));
					oValue.setSize(bValue.length - 8 + bValueTail.length - 8);
					System.arraycopy(bValueTail, 8, oValue.getBytes(), 0, bValueTail.length - 8);
					System.arraycopy(bValue, 8, oValue.getBytes(), bValueTail.length - 8, bValue.length - 8);					
				...
				System.arraycopy(bValue, 8, oValue.getBytes(), 0, bValue.length - 8);
				context.write(oKey, oValue);
			}
		}
	}
