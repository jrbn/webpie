	...
	private static void rewrite(String inputDir, String outputDir) {
		try {
			...
			FileStatus[] files = fs.listStatus(
					new Path(inputDir, "_dict/table"), new OutputLogFilter());
			...
			DataOutputStream indexOut = new DataOutputStream(
					new BufferedOutputStream(new FileOutputStream(outputDir
							+ File.separator + "index.data")));

			DataOutputStream textOut = new DataOutputStream(
					new BufferedOutputStream(new FileOutputStream(outputDir
							+ File.separator + "text.data")));

			long offset = 0;

			for (FileStatus file : files) {
				FileStatus[] tableFiles = fs.listStatus(file.getPath(),
						new OutputLogFilter());
